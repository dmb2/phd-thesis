...
A useful phenomenological model[fn:: The same model Pythia uses in its
simulation of octet J/\psi production \cite{Sjostrand:2007gs}] is to
consider the charmonium system as a color-charged particle which can
couple to gluons using rules of QCD. As this particle propogates it
emits co-linear gluon radiation in the same manner as a quark. When
the composite particle falls below a certain threshold, it transitions
to a color singlet J/\psi hadron and emits the remaining color charge
as a soft gluon which is incorporated into the underlying event. This
is in contrast to color singlet production which has the J/\psi
produced directly and no net color charge.
