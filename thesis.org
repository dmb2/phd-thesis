...
laid the groundwork for what would become quantum chromodynamics (\acrshort{qcd})
...
quantum mechanics and empirical models of the \acrshort{qcd} potential. Even with
...
\acrshort{qcd}. In 1992, the CDF collaboration at the Tevatron reported the first
...
worked out many of the details of quarkonium production in \acrshort{qcd} using
...
#+CAPTION: Feynman rules for \acrshort{qcd}, see \cite{Ellis:1991qj} for more details.
...
interactions. In \acrshort{qcd}, bound states consist of mesons and baryons in
...
make it an ideal system to study properties of \acrshort{qcd}. The bound state
...
strength of the coupling strength of \acrshort{qcd} in the charmonium system is
...
The instantaneous luminosity $L$ of the \acrshort{lhc} is given by the following
formula:
#+BEGIN_LaTeX
\begin{equation}
...
As with other subsystems in ATLAS, the \acrshort{ecal} consists of separate
barrel and end-cap detectors with slightly different geometries.
Figure \ref{fig:ecal-barrel-module} shows a segment of the \acrshort{ecal}
system. The \acrshort{ecal} utilizes an accordion geometry to ensure that more
...
in the $\phi$ direction. The \acrshort{ecal} is finely segmented in $\eta$
($\Delta \eta = 0.0031$) close to the inner detector and less so in
the bulk of the detector ($\Delta \eta = 0.025$). The end of the \acrshort{ecal}
...
decide if the event should be recorded or not. The \acrshort{ecal}'s
...
\cite{atlas-detector}. Lead is used as the absorber in the \acrshort{ecal}. Lead
is an ideal choice due to its high density and the compact nature of
the \acrshort{ecal} compared to the \acrshort{hcal}.
...
in 2.5 $\mu$s. The L1 trigger also creates regions of interest
...
$z_0\sin(\theta) < 1.5$ and $d_0 < 1.0$ are also applied. The final
set of jets that are reconstructed must be central to the detector
$|\eta| < 2.5$ and must have high transverse momentum $p_T > 45$ GeV.
Finally, the $J/\psi$ and the jet must be within $\Delta R < 0.4$ (the
...
weights representing categories parametrized by the \acrshort{pdf}s used in the
\acrshort{ml} fit. SPlot requires that the normalization condition be relaxed
...
\end{equation}
#+END_LaTeX
Where $\sigma_S$ is the variance of the signal \acrshort{pdf}. This weight is
...
predictions from \acrshort{nrqcd} in interesting ways. It may be possible to use
