...
issues, namely a "fudge factor" to scale the numerical calculations
close enough to their experimental values to be compared
...
The extra \acrshort{nrqcd} components that describe the observed transverse
...
The second feature is the self-interaction of the gluon.  In \acrshort{qed}, the
mediating photon has no electric charge, and thus does not couple to
itself.  In \acrshort{qcd}, the gluon carries color charge. This means that the
...
This property of \acrshort{qcd} is referred to as confinement. 
* Quarkonium
...
strength of the coupling strength of \acrshort{qcd} in the charmonium
system is $\alpha_s(m_c)\approx0.24$ \cite{Bodwin:1994jh}. This is
large enough that the perturbation series converges more slowly than
one would hope. The higher mass of the bottom quark allows the
...
self-interact, the \acrshort{qed} potential and the \acrshort{qcd} potential would be the
...
notation:[fn:: referred to as a term symbol] $^{2S+1}L_J^{(1,8)}$ with
...
$\mathcal{O}^H_{8,1}(n)$) is the known long distance matrix element
(LDME).
...
helium explosion and delayed the experiment by a year \cite{lhc-quench}.
...
ellipse which describes the transverse phase-space profile of the \acrshort{lhc}
...
* The ATLAS detector
\acrlong{atlas} (\acrshort{atlas}[fn:: AT\acrshort{lhc}A doesn't have the same ring,
or status as a greek titan.]) is one of the general purpose detectors
at the \acrshort{lhc}. It is located at Point 1 on the \acrshort{lhc} ring. Figure
...
all work on the principle that when a charged particle moves through
...
(\acrshort{trt}). While the pixel and \acrshort{sct} detectors rely on semiconductor
technology, the \acrshort{trt} utilizes ionization that occurs when a charged
...
barrel and end cap \acrshort{trt}. The \acrshort{trt} is constructed from a series of
...
charged particles to ionize when the pass through the \acrshort{trt} volume
...
While the \acrshort{ecal} utilizes a accordion geometry, the \acrshort{hcal} utilizes a
slightly different form. The absorption material in the \acrshort{hcal} is steel,
the scintillation material is a plastic based on polystyrene doped
with fluors \cite{atlas-detector}. The \acrshort{hcal} is separated into the
barrel region referred to as the tile calorimeter, and the forward
region covered by the hadronic end-cap calorimeters (\acrshort{hec}). The
...
these leptons are electrons, they are reconstructed in the \acrshort{ecal} and
...
initiate a bremsstrahlung cascade as it passes through the \acrshort{ecal} and
\acrshort{hcal} \cite{Agashe:2014kda}. This allows it to reach the outer region
...
of monitored drift tubes (\acrshort{mdt}s), cathode strip chambers (\acrshort{csc}s),
resistive plate chambers (\acrshort{rpc}s) and thin gap chambers (\acrshort{tgc}s). Each of
...
*** Thin Gap Chambers
The \acrshort{tgc} serves a similar purpose as the \acrshort{rpc}s do. They are the magenta
regions of Figure \ref{fig:rpc-tgc-trigger}. \acrshort{tgc}s and \acrshort{rpc}s operate
using similar physical processes but differ in the materials used
\cite{atlas-detector}. The region of the \acrshort{tgc} filled with gas is 2.8 mm
...
parameters of the \acrshort{tgc} allow it to read out signals within 25ns
...
into the \acrshort{trt} volume. \acrshort{trt} hits which lie along the reconstructed tracks
are associated with each track. The software requires that adding \acrshort{trt}
...
\ref{fig:trt-ttbar} shows the \acrshort{trt} extension procedure for a simulated
$t\bar{t}$ event at ATLAS, red \acrshort{trt} hits are associated with the tracks
reconstructed in the silicon layers. After \acrshort{trt} hits are associated to
...
appear in the \acrshort{trt} to not be reconstructed during the inside-out
...
amounts of energy in the \acrshort{trt} which makes the gaussian assumption
...
construction of the \acrshort{lhc} means that only 200 events can be written per
...
and are written out to disk \cite{atlas-detector}. This analysis uses
the following trigger chain: =L1_MU15= $\rightarrow$ =L2_mu36_tight=
...
condition be relaxed such that for a given component of the likelihood
function \cite{Barlow:1990vc},
#+BEGIN_LaTeX
\begin{equation}
...
#+BEGIN_LaTeX
      \begin{figure}
        ...
        of parameterization in the ML fit. The resolution function is
        switched with a gaussian. The mass background polynomial is
        varied. The symmetric lifetime component is made asymmetric. The
        crystal ball parameters that are fixed are varied.} 
    \end{figure}
#+END_LaTeX
...
produced in the Fragmenting Jet Function (\acrshort{fjf}) framework compared to
...
available that produces $J/\psi$ using the \acrshort{nrqcd} description of
...
cross sections for \acrshort{nrqcd} quarkonium states in Sherpa. If this work is
