...
embedded radiation as possible.  
...
the alignment of the $J/\psi$ to the surrounding radiation.
...
\ref{fig:uc-dbl-gaus-delta_r} shows the estimated response matrix and
...
Up to this point, the discussion has been centered around explaining
the details that go into this analysis. In this chapter, the selection
criteria (the physical properties required by each event) are
enumerated. A discussion of the systematic errors that goes into the
measurement and how their impact on the various analysis steps is
explained. The results are presented and a discussion and
interpretation of the results follows. This chapter differs from the
previous chapters in that it is more in line with a short note, or
rapid communications than a pedagogical explanation for the choices
that lead to specific decisions.
* Selection Criteria
