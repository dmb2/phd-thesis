...
produce a $Q\bar{Q}$ pair with quantum numbers $n$. The LDME
...
#+BEGIN_LaTeX
  \begin{figure}[htp]
    ...
    \subfloat[$g\rightarrow qg$]{
      \label{fig:g-to-qg}
      \includegraphics[width=0.4\textwidth]{figures/g_to_qg.eps}
    }\\
...
state partons.  The gluon splitting function is:
#+BEGIN_LaTeX
\begin{equation}
\label{eq:gg-split}
...
\end{equation}
#+END_LaTeX
Here $C_A=3$, notice that when one of the gluons is soft either ($c$ soft)
$z\rightarrow 0$ or ($b$ soft) $z\rightarrow 1$.  The first term of
equation \ref{eq:gg-split} is divergent for $z\rightarrow 0$ as well
as $z\rightarrow 1$ for the the second term.  These divergences favor
soft emission of gluons.  
