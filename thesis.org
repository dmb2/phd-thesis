...
theoretical work. The interactions of QCD are determined by an SU(3)
symmetry group. In order to organize which configurations are allowed
by the symmetry a color charge ($a=1,2,3$) is assigned to each quark.
It is useful to assign the colors red, green, and blue to the color
index in order to provide intuition for the dynamics of the interaction.
** Quarks
Table \ref{tab:sm-quarks} shows the three generations of quarks. Each
quark is spin 1/2. Each generation consists of a flavor doublet with
charge 2/3 and charge -1/3. Quarks with charge 2/3(-1/3) are referred
to as up(down)-type quarks. The particle which mediates the strong
interaction is the spin 0 gluon.
