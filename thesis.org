...
(PDF)[fn:: This section follows the notation in \ref{James:2006zz}
...
\begin{equation}
\label{eq:nll}
- \log \mathcal{L} = - \sum_{i=1}^{N} \log(\sum_{j=1}^\mathcal{N}_{s} N_j p_j(x_i|\bm{\theta}))
\end{equation}
#+END_LaTeX
...
to reconstruct a histogram of the estimated signal. Define a histogram
$\bm{H}$ as a vector of dimension $n$ corresponding to the number of
bins, and $H_i$ is the $i^{\text{th}}$ bin. The sPlot method provides
the following decomposition for the $i^{\text{th}}$ bin that is
statistically independent of the discriminating variable $x$:
#+BEGIN_LaTeX
\begin{equation}
H_i = (W_S + W_B)N
\end{equation}
#+END_LaTeX
In this expression, $N$ is the number of entries in that bin.  From
this it is easy to see that the histogram of the signal process will
be:
#+BEGIN_LaTeX
\begin{equation}
H^S_i = W_S N
\end{equation}
#+END_LaTeX
In order to derive this weight, introduce the probability that a
signal(background) event occurs in bin $i$ as $p_{S(B)}$. If the
number of signal (background) events from the overall fit is
$N_{S(B)}$, then the expected number of signal events in bin $i$
should be $p_S N_S$.  This can be written in terms of the weights
above with the substitution $N_S = \int p_S(x) dx$:
#+BEGIN_LaTeX
\begin{equation}
p_S N_S = \int W_S (p_S p_S(x) + p_B p_B(x)) dx
#+END_LaTeX
