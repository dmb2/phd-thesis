...
** Quark Flavor
In addition to carrying color charge, quarks carry a flavor. Table
\ref{tab:sm-quarks} shows the three generations of quarks. Each
...
#+CAPTION: The three generations of quarks in the Standard Model 
#+LABEL: tab:sm-quarks
| Generation | I    | II      | III    |
|------------+------+---------+--------|
| /          | <    |         |        |
...
We begin by writing the QCD lagrangian in terms of a classical field
plus a gauge fixing term, and a ghost term.  
#+BEGIN_LaTeX
  \begin{equation*}
...
term which distinguishes QCD from quantum electrodynamics. In order to
derive the Feynman rules for QCD, we must evaluate the path integral
of the action. In the standard formulation of the action, the
resulting integral is badly divergent because of the gauge invariance
of the theory \cite{Peskin:1995ev} [fn:: See 9.4 Quantization of the
Electromagnetic Field]. Following the Faddeev-Popov method of gauge
fixing used in \cite{Ellis:1991qj} the following two terms are added
to the lagrangian:
#+BEGIN_LaTeX
  \begin{align}
...
terms of a free field lagrangian $S₀$ and an interacting term $S_I$:
#+BEGIN_LaTeX
  \begin{equation*}
  S = S₀ + S_I
  \end{equation*}
#+END_LaTeX
Further we identifying $∂ᵅ = -ipᵅ$ allows us to write down the two
point function for the quarks and gluons, resulting in the Feynman
rules shown in Figure \ref{fig:qcd-feyn-rules}. 
...
interactions allowed by the theory of QCD.  
