...
summarized in \ref{tab:data-sets}.
...
tract based observables: the scale of the track, the resolution of the
track and the efficiency to reconstruct the track. Muons are further
complicated by the fact that they are the estimate of two detector
subsystems, the inner detector and the muon spectrometer.  Therefore
there are systematic effects associated with each of these
...
the difference from the nominal result is 
* Results
* Discussion
