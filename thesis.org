...
\cite{atlas-detector}.  The region of the TGC filled with gas is 2.8
mm wide.  The gas mixture is CO_2 and n-pentane which results in a
lower gas gain.  The anodes are held at a potential of 2.9 kV.  The
operating parameters of the TGC allow it to read out signals within
25ns \cite{atlas-detector}.  
...
parametrization difficult, therefore a generic set of parameters is
...
the Standard Model. The rate of this process is quite long which
allows the experimental discrimination of events containing a $b$
quark. The long lifetime of $b$ flavored mesons means that the decay
vertex is displaced from the primary vertex. By measuring the distance
between the refitted vertex and the primary vertex of the event, an
estimate of the lifetime of the particle can be made. The lifetime can
be estimated by measuring the displacement vector from the primary
vertex ($\mathbf{L}$) and projecting it on the transverse momentum
vector ($\mathbf{p}_T$). This quantity is referred to as $L_{xy}$.
Determining the pseudo-proper lifetime is then a matter of scaling
this quantity by the ratio of the mass and transverse momentum of the
particle[fn:: In the high energy limit, the relativistic gamma factor
is proportional to the ratio $p_T/m$]:
#+BEGIN_LaTeX
\begin{align}
...
The maximum likelihood fit method is a statistical method for
estimating parameters which describe the probability density function
(PDF). The PDF can be motivated by physical first principles or
empirically. The likelihood function is defined as the probability of
a set of parameters given observed data. Maximizing the likelihood
function with respect to these parameters provides a best estimate of
the parameters. The likelihood function is constructed as a product
over probability functions for observing a random variable $x$
\cite{Barlow:1990vc}:
#+BEGIN_LaTeX
\begin{equation}
\mathcal{L} = \prod_{i=1}^N p(x_i ; a_1 \cdots a_m)
\end{equation}
#+END_LaTeX
Here $N$ is the number of observed events and $a_1 \cdots a_m$ are the
parameters of the PDF which describe the data. Each probability
function $p(x; a_1 \cdots a_m)$ is normalized \cite{Barlow:1990vc}:
#+BEGIN_LaTeX
\begin{equation}
\int p(x; a_1 \cdots a_m) = 1
\end{equation}
#+END_LaTeX
Rather than maximizing the likelihood directly, it is easier to
maximize the logarithm of the likelihood.  It is also equivalent to
minimizing the negative logarithm of the likelihood function:
#+BEGIN_LaTeX
\begin{equation}
\label{eq:nll}
- \log \mathcal{L} = - \sum_{i=1}^{N} \log(p(x_i; a_1 \cdots a_m))
\end{equation}
#+END_LaTeX
Equation \ref{eq:nll} is minimized over the parameters $a_j$. This is
done using RooFit, a computational framework that implements these
mathematical functions in code directly \cite{Verkerke:2003ir}. The
algorithm used by RooFit is a gradient descent called Minuit
\cite{James:2004xla}. The method used to separate the signal and
background events requires that the normalization condition be relaxed
such that for a given component of the likelihood function
\cite{Barlow:1990vc}:
#+BEGIN_LaTeX
\begin{equation}
\int P(x; a_1 \cdots a_m) dx = N(a_1 \cdots a_m)
\end{equation}
#+END_LaTeX
This allows the derivation of per event weights that describe the
signal and background components for each variable of interest to this
analysis. The details of this method are described in Section [[SPlot]].
* Empirical Model
