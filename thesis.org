...
parameters of the PDF which describe the data. It is important to note
that the parameters $a_i$ do not represent a conditional probability.
Instead, they are parameters of a function describing the PDF[fn:: For
example, in the function of a line $p(x; m, b)=mx+b$, the slope ($m$)
and intercept of a line ($b$) are parameters of the variable $x$.],
independent of the random variable observed ($x_i$). Each probability
...
\end{equation}
#+END_LaTeX
Equation \ref{eq:nll} is minimized over the parameters $a_j$. The
...
SPlot requires that the normalization condition be relaxed such that
for a given component of the likelihood function \cite{Barlow:1990vc}:
#+BEGIN_LaTeX
\begin{equation}
\int P(x; a_1 \cdots a_m) dx = N(a_1 \cdots a_m)
\end{equation}
#+END_LaTeX
In addition SPlot also requires the covariance matrix derived from the
fit.  This also goes under the name "information matrix" in the
literature and it is defined as:
#+BEGIN_LaTeX
\begin{equation}
